...
from omniplay.common.enums import ObservationType, OutputStrategies
...
from omniplay.llm import LLM, LLMCallOptions, LLMMessage, LLMResponse, ModelConfig, Provider
...
class LLMParams(PlayerParams):
    ...
    def from_string(cls, params_string: str) -> LLMParams:
        ...
        if len(parts) < 4:
            raise ValueError(
                f'Invalid LLM params: {params_string!r} (need observation:strategy:provider:model[:options])')
...
class LLMPlayer(Player):
    ...
    async def __call__(self, game: TurnBasedGame, observation: InterfaceObservation, legal_moves: list[InterfaceAction]) -> PlayerOutput:
        ...
        response = await self._llm.generate(
            self._params.model, system_message, messages, output_schema=self._output_strategy.get_output_schema()
        )
